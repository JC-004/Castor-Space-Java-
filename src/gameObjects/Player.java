...
public class Player extends MovingObject{
	...
	public Player(Vector2D position, Vector2D velocity,
                double maxVel, BufferedImage texture, GameState gameState) {
		...
		fastFireTime = 0;
		doubleGunTime = 0;
                //INICIZALIZA EL SONIDO DE DISPARO
                shoot = new Sound(Assets.playerShoot);
                
                
                //INICIALIZA EL SONIDO DE CHOQUE
	...
	public void update(float dt){
		...
		if(KeyBoard.SHOOT &&  fireRate > fireSpeed && !spawning)
		{
			...
			}else {//CON LO DE ABAJO CONTROLAMOS LA VELOCIDAD DE DISPARO
				gameState.getMovingObjects().add(0,new Laser(
						getCenter().add(heading.scale(width)),
						heading,
                                                //LO DE ABAJO ES LA VELOCIDAD DEL LASER
						Constants.LASER_VEL,
						angle,
                                                //ESTO ES EL COLOR DE LA BALA DEL JUGADOR
						Assets.redLaser,
						gameState
						));
			}
			...
			shoot.play();
                ...
                //8500
                //PARA OBTENER EL #7786 SE UTILIZO UNA CLASE PARA CALCULAR EL NUMERO DE FRAMES
		if(shoot.getFramePosition() > 7786) {
			shoot.stop();
		}
		//ACA SE DEFINE LA ROTACION
		if(KeyBoard.RIGHT)
	...
	public void Destroy() {
		...
		spawnTime = 0;
                //ACA SE INICIA
                loose.changeVolume(-20.0f);
		loose.play();
                //ACA SE MODIFICARIA EL SONIDO DE CHOQUE
